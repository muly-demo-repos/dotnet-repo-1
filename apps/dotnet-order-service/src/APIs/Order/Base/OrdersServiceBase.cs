...
public abstract class OrdersServiceBase : IOrdersService
{
    ...
    public OrdersServiceBase(DotnetOrderServiceDbContext context)
    {
        _context = context;
    }
    ...
    /// </summary>
    public async Task ConnectPayments(OrderIdDto idDto, PaymentIdDto[] paymentsId)
    {
        var order = await _context
            .Orders.Include(x => x.Payments)
            .FirstOrDefaultAsync(x => x.Id == idDto.Id);
        ...
        var paymentsToConnect = payments.Except(order.Payments);
    ...
    /// </summary>
    public async Task DisconnectPayments(OrderIdDto idDto, PaymentIdDto[] paymentsId)
    {
        var order = await _context
            .Orders.Include(x => x.Payments)
            .FirstOrDefaultAsync(x => x.Id == idDto.Id);
    ...
    /// </summary>
    public async Task<List<PaymentDto>> FindPayments(
        OrderIdDto idDto,
        PaymentFindMany orderFindMany
    )
    {
        var payments = await _context
            .Payments.Where(m => m.OrderId == idDto.Id)
            .ApplyWhere(orderFindMany.Where)
            .ApplySkip(orderFindMany.Skip)
            .ApplyTake(orderFindMany.Take)
            .ApplyOrderBy(orderFindMany.SortBy)
    ...
    /// </summary>
    public async Task UpdatePayments(OrderIdDto idDto, PaymentIdDto[] paymentsId)
    {
        var order = await _context
    ...
    public async Task UpdateOrder(OrderIdDto idDto, OrderUpdateInput updateDto)
    {
        ...
        if (updateDto.Payments != null)
        {
            order.Payments = await _context
                .Payments.Where(payment =>
                    updateDto.Payments.Select(t => t.Id).Contains(payment.Id)
                )
                .ToListAsync();
        }
